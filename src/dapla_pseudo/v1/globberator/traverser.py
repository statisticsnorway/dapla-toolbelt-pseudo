...
class SchemaTraverser:
    ...
    def from_path(schema_path: str, rules_path: str) -> "SchemaTraverser":
        ...
        schema_opener = GCSFileSystem().open if schema_path.startswith("gs://") else open
    ...
    def match_rules(self, separator: str ="/") -> list[PseudoRule]:
        ...
        """
        return list(SchemaTraverser._match_rules(self.schema, self.rules, "", separator))
    
    @staticmethod
    def _match_rules(
        schema: dict[str, pl.DataType], rules: list[PseudoRule], prev_path: str, separator: str
